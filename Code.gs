...
/**
 * Publishes the current presentation and returns a shareable link.
 * Triggered by the "Publish" button in the sidebar.
 */
function publishPresentation() {
  try {
    ...
    // Generate a unique publication ID (used in the URL)
    const publicationId = Utilities.getUuid().substring(0, 12);
    
    // Store the publication data in script properties
    const scriptProps = PropertiesService.getScriptProperties();
    const publicationsJson = scriptProps.getProperty('PUBLICATIONS') || '{}';
    const publications = JSON.parse(publicationsJson);
    
    // Add this publication to the list
    ...
    scriptProps.setProperty('PUBLICATIONS', JSON.stringify(publications));
    
    // Generate the sharable URL
    let shareableUrl;
    
    // Check if we have a master webapp ID set
    const masterWebAppUrl = getMasterWebAppUrl();
    if (masterWebAppUrl) {
      // Use the existing master webapp URL structure
      shareableUrl = `${masterWebAppUrl}?pub=${publicationId}`;
    } else {
      // Fallback to using presentationId directly (less secure/optimal)
      shareableUrl = `${MASTER_PUBLISHING_SERVICE.URL}?presId=${presentationId}`;
    }
    ...
    console.error("Error in publishPresentation:", e);
    return {
      success: false,
...
/**
 * Gets a previously published presentation by ID
 */
function getPublishedPresentation(publicationId) {
  try {
    const scriptProps = PropertiesService.getScriptProperties();
    const publicationsJson = scriptProps.getProperty('PUBLICATIONS') || '{}';
    const publications = JSON.parse(publicationsJson);
    
    // If publication exists, update last accessed timestamp
    if (publications[publicationId]) {
      publications[publicationId].lastAccessed = new Date().toISOString();
      scriptProps.setProperty('PUBLICATIONS', JSON.stringify(publications));
    ...
    console.error("Error in getPublishedPresentation:", e);
    return {
      success: false,
      error: `Error retrieving publication: ${e.message}`
    };
  }
}
